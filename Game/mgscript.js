...
function startGame() {
    ...
    document.getElementById('problem').innerText = `${currentProblem.num1} ${currentProblem.operation} ${currentProblem.num2} = ?`;
...
function startTimer() {
    ...
    timerInterval = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            ...
            document.getElementById('result').innerText = `Time's up! The correct answer was ${currentProblem.correctAnswer}. You answered ${correctAnswers} questions correctly.`;
...
function endGame() {
    ...
    document.getElementById('continue').style.display = 'none';
    document.getElementById('result').innerText = `You answered ${correctAnswers} questions correctly.`;
...
function checkAnswer(selectedOption) {
    clearInterval(timerInterval); // Stop the timer
    if (selectedOption == currentProblem.correctAnswer) { // Use == for loose comparison (to handle number/string mismatch)
        ...
        document.getElementById('result').innerText = `Incorrect. The correct answer is ${currentProblem.correctAnswer}. You answered ${correctAnswers} questions correctly.`;
